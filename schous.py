...
st.sidebar.markdown("---")
_safe_db = re.sub(r"://([^:]+):([^@]+)@", r"://\1:***@", _db_url) if "://" in _db_url else _db_url
...
# -----------------------------
st.sidebar.markdown("---")
st.sidebar.caption("Brewery Manager v2.0 • Multiusuário (Postgres/SQLite)")# Brewery Manager - Multi-user Streamlit app (Postgres/Neon)
import streamlit as st
import bcrypt
import pandas as pd
import numpy as np
...
_safe_db = re.sub(r"://([^:]+):([^@]+)@", r"://\1:***@", _db_url) if "://" in _db_url else _db_url
st.sidebar.info(f"🗄️ Banco: {_safe_db}")
st.sidebar.info(f"👤 Perfil: {st.session_state.get('auth_role','viewer')}")
st.sidebar.info(f"📁 Tabelas carregadas: {len(data)}")
